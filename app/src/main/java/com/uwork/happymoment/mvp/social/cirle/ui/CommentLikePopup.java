package com.uwork.happymoment.mvp.social.cirle.ui;
...
import com.example.circle_base_library.thirdpart.WeakHandler;
 ...
 * 朋友圈点赞
 */
public class CommentLikePopup extends BasePopupWindow implements View.OnClickListener {
    ...
    private MomentItemBean mMomentsInfo;
    ...
    public CommentLikePopup(Activity context) {
...
//        setBlurBackgroundEnable(true);
    ...
    public void updateMomentInfo(@NonNull MomentItemBean info) {
        this.mMomentsInfo = info;
        hasLiked = false;
        if (!ToolUtil.isListEmpty(info.getLikesList())) {
            for (MomentLikeBean bean : info.getLikesList()) {
                if (bean.getId() == UserManager.getInstance().getUserId(getContext())) {
                    hasLiked = true;
                    break;
                }
            }
    ...
    public interface OnCommentPopupClickListener {
        void onLikeClick(View v, @NonNull MomentItemBean info, boolean hasLiked);

        void onCommentClick(View v, @NonNull MomentItemBean info);
    }
